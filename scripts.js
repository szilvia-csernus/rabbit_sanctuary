...
Array.from(myForms).forEach(function (form) {
    form.addEventListener("submit", function (event) {
        // if none of the input fields are :invalid - meaning they all
        // passed Bootstrap"s
        ...
        if (invalidInputs.length === 0) {
            ...
            event.preventDefault();
            // ...apply data-bs-toggle and data-bs-target attributes to
            // form so that the click event will trigger
            // my-thank-you-modal to appear
            form.dataset.bsToggle = "modal";
            form.dataset.bsTarget = "#my-thank-you-modal";
            // ...inintiate the click() event
            form.click();
            // ...reset all fields and attributes so they don"t linger for
            // next time.
...
// custom script to close sidebar after clicking "Donate" button -
// otherwise it would stay open even after we sent off the Donate form.

const myDonateBtn = document.getElementById("my-donate-btn");
myDonateBtn.addEventListener("click", function () {
    // when "Donate" was reached from sidebar
    if (window.innerWidth < 768) {
        // by clicking the burger icon again, we initiate "toggling"
        // which will close sidebar.
